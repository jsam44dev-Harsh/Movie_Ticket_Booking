...
const Schedule = () => {
	...
	return (
		<div className="text-white flex min-h-screen flex-col gap-4 pb-8  sm:gap-8" style={{
			...
			{selectedCinemaIndex !== null &&
				(cinemas[selectedCinemaIndex]?.theaters?.length ? (
					<div className="text-white border-solid border-2 border-green-900 mx-4 flex flex-col gap-2 rounded-lg bg-black p-4 drop-shadow-xl sm:mx-8 sm:gap-4 sm:p-6">
						<h2 className="text-3xl font-bold text-white flex justify-center">Schedule</h2>
						<DateSelector selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
						{auth.role === 'admin' && (
							<form
								className="flex flex-col border-solid border-2 border-[#1B4242] lg:flex-row gap-4 rounded-md bg-black p-4"
							...
							>
								<div className="flex grow flex-col gap-2 rounded-lg">
									<div className="flex flex-col gap-2 rounded-lg lg:flex-row lg:items-stretch">
										<div className="flex grow items-center gap-x-2 gap-y-1 lg:flex-col lg:items-start">
											...
											<select
												className="h-9 w-full rounded bg-gray-500 px-2 py-1 font-semibold text-gray-900 drop-shadow-sm"
												required
										...
										<div className="flex grow-[2] items-center gap-x-2 gap-y-1 lg:flex-col lg:items-start">
											...
											<Select
												...
												classNames={{
													menuButton: (value) =>
														' bg-gray-600 flex font-semibold text-sm border border-gray-300 rounded shadow-sm transition-all duration-300 focus:outline-none bg-white hover:border-gray-400 focus:border-indigo-500 focus:ring focus:ring-indigo-500/20'
												}}
											/>
										</div>
										<div className="flex items-center gap-x-2 gap-y-1 lg:flex-col lg:items-start">
											...
											<input
												type="time"
												className="h-9 w-full rounded bg-gray-500 px-2 py-1 font-semibold text-white drop-shadow-sm"
												required
								...
								<button
									...
									className="whitespace-nowrap rounded-md bg-[#5C8374] px-2 py-1 font-medium text-white drop-shadow-md hover:from-indigo-500 hover:to-blue-400 disabled:from-slate-500 disabled:to-slate-400"
									type="submit"
								>
									ADD +
								</button>
					...
					<div className=" mx-4 flex flex-col gap-2 rounded-lg bg-black border-solid border-2 border-green-900 p-4 drop-shadow-xl sm:mx-8 sm:gap-4 sm:p-6">
