...
const User = () => {
	...
	return (
		<div className="flex min-h-screen flex-col gap-4  pb-8 text-gray-900 sm:gap-8" style={{
			...
			<Navbar />
			<div className="mx-4 flex h-fit flex-col gap-2 rounded-lg bg-black border-solid border-2 border-green-900  p-4 drop-shadow-xl sm:mx-8 sm:p-6">
				<h2 className="text-3xl font-bold flex justify-center text-white">Users</h2>
				...
				<div
					className={`mt-2 grid max-h-[60vh] overflow-auto rounded-md bg-gradient-to-br from-indigo-100 to-white border-solid border-2 border-green-900`}
					style={{ gridTemplateColumns: 'repeat(3, minmax(max-content, 1fr)) max-content max-content' }}
				>
					<p className="sticky top-0  px-2 py-1 text-center text-xl font-semibold text-black bg-[#5C8374]">
						Username
					</p>
					<p className="sticky top-0  px-2 py-1 text-center text-xl font-semibold text-black bg-[#5C8374]">
						Email
					...
					{users
						...
						.map((user, index) => {
							return (
								<Fragment key={index}>
									<div className="border-t-2 flex justify-center border-indigo-200 px-2 py-1 bg-black text-white">{user.username}</div>
									<div className="border-t-2 flex justify-center border-indigo-200 px-2 py-1 bg-black text-white">{user.email}</div>
									<div className="border-t-2 flex justify-center border-indigo-200 px-2 py-1 bg-black	 text-white">{user.role}</div>
									<div className="border-t-2 flex justify-center border-indigo-200 px-2 py-1 bg-black text-white">
										<button
		...
		${ticketsUser === user.username
									...
									<div className="flex gap-2 border-t-2 border-indigo-200 px-2 py-1 bg-black">
										{user.role === 'user' && (
											<button
												className="flex w-[115px] items-center justify-center gap-1 rounded bg-[#5C8374] py-1 pl-2 pr-1.5 text-sm font-medium text-white hover:from-indigo-500 hover:to-blue-400 disabled:from-slate-500 disabled:to-slate-400"
