...
const TheaterListsByMovie = ({ movies, selectedMovieIndex, setSelectedMovieIndex, auth }) => {
	...
	return (
		<>
			<CinemaLists {...props} />
			<div className="mx-4 h-fit rounded-md bg-black border-solid border-2 border-green-900 to-blue-100 text-gray-900 drop-shadow-md sm:mx-8">
				<div className="flex flex-col gap-6 p-4 sm:p-6">
					<DateSelector selectedDate={selectedDate} setSelectedDate={setSelectedDate} />
					<div className="flex flex-col gap-4 rounded-md bg-[#5C8374]  py-4">
							...
							{filteredTheaters.map((theater, index) => {
								return (
									...
									>
										{filteredTheaters[index - 1]?.cinema.name !==
											filteredTheaters[index].cinema.name && (
											<div className="rounded-t-md bg-[#538392] px-2 py-1.5 text-center text-2xl font-semibold text-white sm:py-2">
