...
const CinemaLists = ({
		...
		return cinemasList.length ? (
			cinemasList.map((cinema, index) => {
				return cinemas[selectedCinemaIndex]?._id === cinema._id ? (
					<button
						className="w-fit rounded-md bg-[#5C8374] px-2.5 py-1.5 text-lg font-medium text-white drop-shadow-xl border-solid border-2 border-red-500"
						onClick={() => {
					...
					<button
						className="w-fit rounded-md bg-[#5C8374] px-2 py-1 font-medium text-white drop-shadow-md "
						onClick={() => {
	...
	return (
		<>
			<div className="mx-4 mb-10 flex h-fit flex-col gap-4 rounded-md bg-[#000000] p-4  drop-shadow-xl sm:mx-8 sm:p-6 border-solid border-2 border-green-900 text-white">
				<form
					...
					{auth.role === 'admin' && (
						<div className="flex w-fit grow sm:justify-end ">
							<input
								...
								className="w-full grow rounded-l border text-black border-gray-300 px-3 py-1 sm:max-w-xs"
								required
								{...register('name', { required: true })}
							/>
							<button
								disabled={isAdding}
								className="flex items-center whitespace-nowrap rounded-r-md bg-[#5C8374] px-2 py-1 font-medium text-white hover:from-indigo-500 hover:to-blue-400 disabled:from-slate-500 disabled:to-slate-400"
							>
