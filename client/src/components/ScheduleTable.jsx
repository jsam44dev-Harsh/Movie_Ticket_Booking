...
const ScheduleTable = ({ cinema, selectedDate }) => {
	...
	return (
		<>
			<div
				className={`border-solid border-2 border-green-900 grid min-h-[50vh] max-h-screen overflow-x-auto grid-cols-${cinema.theaters?.length.toString()} grid-rows-${
					gridRows + shiftEnd
				} rounded-md bg-black`}
				{...events}
				ref={ref}
			>
								...
								<button
									title={`${showtime.movie.name}\n${new Date(showtime.showtime)
										...
										.padStart(2, '0')}
												`}
									key={index}
									className={`bg-[#5C8374]  flex flex-col items-center overflow-y-scroll row-span-${getRowSpan(
				...
				{cinema.theaters.map((theater, index) => (
					<div
						key={index}
						className="border-solid border-2 border-green-900 sticky top-0 row-span-1 row-start-1 flex flex-col items-center justify-center bg-gradient-to-br from-gray-800 to-gray-700 py-1 text-white"
					>
